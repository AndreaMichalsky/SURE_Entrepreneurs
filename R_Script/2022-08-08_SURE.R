...
# Dropping employment variables by name
sureDF[c("Self_employed_Part_Time", "Self_employed_Full_Time", "Employed_by_an_employer_Part_Time",
         "Employed_by_an_employer_Full_Time", "Freelance_Gig_Work")] <- list(NULL)
 
# Convert logic variables to factors
...
## Bar Chart of business launched
jpeg("Percent_launched.jpg", width = 140, height = 140, units = "mm", res = 300)
barplot(summary(factor(sureDF$launched))/1808*100, names = c("Not launched", "Launched"), ylab = "Small Business Launch Rate (%)", cex.lab = 1,
        cex.names = 1, las = 1, col = c("red2","darkgreen"), ylim = c(0,100))
dev.off()
table(sureDF$launched)

# Partitioning the data into two sets
set.seed(1)
train.index <- sample(row.names(sureDF), 0.6*dim(sureDF)[1], prob = ifelse(sureDF$launched==1, 0.5, 0.5))  # Taking 60% of the data as the training data set. As the software does not learn much about zeros, I re-sampled the data putting equal weight on 0 and 1)
#valid.index <- sample(row.names(sureDF), 0.4*dim(sureDF)[1], prob = ifelse(sureDF$launched==1, 0.5,0.5)) # Taking 40% of the data as the test data set
valid.index <- setdiff(row.names(sureDF), train.index) 
...
prp(pruned.ct, type = 1, extra = 1, under = TRUE, split.font = 1, varlen = -10, tweak = 1.5, space = 0, gap = 0, compress= TRUE, ycompress = TRUE,
    box.col=ifelse(pruned.ct$frame$var == "<leaf>", 'gray', 'white'))
# count number of leaves for pruned.ct
length(pruned.ct$frame$var[pruned.ct$frame$var == "<leaf>"])

####Best-pruned tree####
pruned.ct.best <- prune(cv.ct, cp = 0.00481799)
prp(pruned.ct.best, type = 1, extra = 1, under = TRUE, split.font = 1, varlen = -15, tweak = 1.5, space = 0, gap = 0, compress= TRUE, ycompress = TRUE, 
    box.col=ifelse(pruned.ct.best$frame$var == "<leaf>", 'gray', 'white')) 
# count number of leaves 
length(pruned.ct.best$frame$var[pruned.ct.best$frame$var == "<leaf>"])

# Confusion Matrix for Pruned Tree using CP lowest xerror
cm.ptlowerr <- predict(pruned.ct, valid.df)
confusionMatrix(as.factor(round(cm.ptlowerr[,2])), as.factor(valid.df$launched))

# confusion Matrix for best pruned tree
cm.besttree <- predict(pruned.ct.best, valid.df)
confusionMatrix(as.factor(round(cm.besttree[,2])), as.factor(valid.df$launched))
...
train.df_imputed$launched <- as.factor(train.df_imputed$launched)

set.seed(1)
boost <- boosting(launched ~ ., data = train.df_imputed)
pred <- predict(boost, valid.df)
confusionMatrix(as.factor(pred$class), as.factor(valid.df$launched))


###################################################################
# Logistic Regression (y = launched) 
###################################################################
#install.packages("tidyverse")
#install.packages("modeldata")
library(tidyverse)
library(modeldata)
# Partitioning the sample into three groups (Group 1: pre-2016, Group 2: 2016+2017+2018, and Group 3: 2019 and later)
class(sureDF$Year_Enrollment)

sureDF_g1 = sureDF[sureDF$Year_Enrollment < 2016,]
sureDF_g2 = sureDF[sureDF$Year_Enrollment %in% 2016:2018,]
sureDF_g3 = sureDF[sureDF$Year_Enrollment > 2018,]

# Tables
xtabs(~ launched + Gender, data=sureDF_g1)
xtabs(~ launched + Hispanic, data=sureDF_g1)
xtabs(~ launched + Race, data=sureDF_g1)
xtabs(~ launched + Martial_Status, data=sureDF_g1)
xtabs(~ launched + Education_Level, data=sureDF_g1)
xtabs(~ launched + Household_Size, data=sureDF_g1)
xtabs(~ launched + Household_Income, data=sureDF_g1)


# Logistic Regression 
model_g1 = glm(launched ~ Gender + Hispanic + Race + Martial_Status + Education_Level + 
             Household_Size + Household_Income, family = "binomial", data = sureDF_g1)
summary(model_g1)


model_g2 = glm(launched ~ Gender + Hispanic + Race + Martial_Status + Education_Level + 
                 Household_Size + Household_Income, family = "binomial", data = sureDF_g2)
summary(model_g2)


model_g3 = glm(launched ~ Gender + Hispanic + Race + Martial_Status + Education_Level + 
                 Household_Size + Household_Income, family = "binomial", data = sureDF_g3)
summary(model_g3)

###################################################################
# Survival Analysis with R
###################################################################
## Preparing the data set for survival analysis
...
# Replacing blank & space by NA
original.df <- data
original.df[original.df == "" | original.df == " "] <- NA
View(original.df)
is.na(original.df)

# creating a new data frame 
new.df = original.df[,c("Launch_Year", "Business_Active")]
library(dplyr)
new.df$row_ID = 1:nrow(new.df)


## Loading sureDF data set from CSV file
sureDF.sa <- read.csv("sureDF.csv", stringsAsFactors = TRUE)
str(sureDF.sa)
summary(sureDF.sa)

# For loop to convert index 1 to NA
for(i in 1:ncol(sureDF.sa)){
  levels(sureDF.sa[,i])[which(levels(sureDF.sa[,i]) == "")] = NA
}
summary(sureDF.sa)

# Eliminate variables with more than 25% missing
sureDF.sa[,c("Seeking_New_Employment", "First_In_Family_In_College", "Ever_Food_Stamp_Or_SNAP", "age",
          "Military_Service", "Ever_Third_Ward", "product_service", "Primary_Industry", "English_First_Language")] = NULL

sureDF.sa$row_ID = 1:nrow(sureDF.sa)

# Merging two data sets
merge.df = merge(sureDF.sa, new.df, by = "row_ID")  

# Creating a data set for all entrepreneurs who did launched a small business
summary(merge.df$launched)
survival.df = merge.df[merge.df$launched == "Yes",]

# Creating "t" variable (t: Survival time in years)
survival.df$t <- 2021 - survival.df$Launch_Year    
hist(survival.df$t)
summary(survival.df$t)
# Creating "status" variable (Status: Censoring status)
# Status indicates whether the small business' closure was observed (status = 1) or that the survival time was censored (status = 0)
summary(survival.df$Business_Active)
survival.df$Status <- ifelse(survival.df$Business_Active == "FALSE", 1,0)
str(survival.df$Status)
survival.df$SURE_Graduate <- ifelse(survival.df$SURE_Graduate == "TRUE", 1,0)
#survival.df$Status <- as.factor(survival.df$Status)
#summary(survival.df$Status)

# Dropping unnecessary variables
survival.df[c("Entrepreneurs.Info.EntrepreneurID", "Media_Mention", "Filing_Taxes_Method")] <- list(NULL)
View(survival.df)
#summary(as.factor(survival.df$t))

survival.df[!is.na(survival.df$t),]
dim(survival.df)

survival.df2 = survival.df[!is.na(survival.df$t),]
str(survival.df2)
survival.df3 = survival.df2[survival.df2$Launch_Year > 2000,]
summary(survival.df3$t) 

# Dropping employment variables by name
survival.df3[c("Self_employed_Part_Time", "Self_employed_Full_Time", "Employed_by_an_employer_Part_Time",
         "Employed_by_an_employer_Full_Time", "Freelance_Gig_Work")] <- list(NULL)
colnames(survival.df3)
str(survival.df3)


# To answer question 1:
summary(survival.df3$Business_Active)
## Bar Chart of small business that survive 
jpeg("Percent_survive.jpg", width = 140, height = 140, units = "mm", res = 300)
barplot(summary(factor(survival.df3$Business_Active))/383*100, names = c("Failure", "Success"), ylab = "Small Business survival (%)", cex.lab = 1,
        cex.names = 1, las = 1, col = c("darkgray","yellow2"), ylim = c(0,100))
dev.off()


## Histogram of survival time in years 
jpeg("Hist_Survival_time.jpg", width = 140, height = 140, units = "mm", res = 300)
hist(survival.df3$t, xlab= "Survival time in years", main= NULL, col= "yellow2", cex.lab = 1, cex.names = 1, las = 1)
dev.off()
summary(survival.df3$t)
#-----------------------------------------------------------------------------

# Running the model of survival analysis in R
...
time <- t
event <- Status
x <- cbind(SURE_Graduate, Hispanic, Martial_Status, Household_Size, Year_Enrollment, Gender,
             Race, Education_Level, Household_Income)
group <- Hispanic


x2 <- cbind(Education_Level,Household_Income, Race)
...
summary(kmsurvival)
autoplot(kmsurvival, xlab ="Time", ylab="Survival Probability")

# Kaplan-Meir non-parametric analysis by group
kmsurvival1 <- survfit(Surv(time, event) ~ group)
summary(kmsurvival1)
autoplot(kmsurvival1, xlab= "Time (in years)", ylab="Survival Probability", main = "KM-Model")
...
coxph2 <- coxph(Surv(time, event) ~ x2, method="breslow")
summary(coxph2)
...
# The variable t records survival time
# Status indicates whether the small business' clousure was observed (status =1) or that survival time was censored (status =0)
# Survival is the probability of not die. What is the probability of survival beyond t years?
# 96.5% of small business survive beyond 1 year. 
km <- with(survival.df3, Surv(t, Status))
head(km,80)                               # Note that a "+" after the time in the print out of km indicates censoring 

km_fit <- survfit(Surv(t, Status) ~ 1, data = survival.df3)
summary(km_fit) # Kaplan-Meier estimates of the probability of survival over time. 
autoplot(km_fit)
km_fit
# Next, we look at survival curves by Hispanic
str(survival.df3$Hispanic)
km_his_fit <- survfit(Surv(t, Status) ~ Hispanic, data = survival.df3)
summary(km_his_fit)
autoplot(km_his_fit)

# Looking at survival curves by Race
str(survival.df3$Race)
km_rc_fit <- survfit(Surv(t, Status) ~ Race, data = survival.df3)
summary(km_rc_fit)
autoplot(km_rc_fit)

# Looking at survival curves by Gender
str(survival.df3$Gender)
km_gen_fit <- survfit(Surv(t, Status) ~ Gender, data = survival.df3)
summary(km_gen_fit)
plot(km_gen_fit)


# Looking at survival curves by Household income
str(survival.df3$Household_Income)
km_hldincome_fit <- survfit(Surv(t, Status) ~ Household_Income, data = survival.df3)
summary(km_hldincome_fit)
autoplot(km_hldincome_fit)
...
# Fit Cox Model
colnames(survival.df3)
cox <- coxph(Surv(t, Status) ~ SURE_Graduate + Hispanic + Martial_Status + Household_Size + Year_Enrollment + Gender +
               Race + Education_Level + Household_Income, data = survival.df3)
summary(cox)
cox_fit <- survfit(cox)
autoplot(cox_fit)
...
str(survival.df3)
summary(survival.df3$Household_Income)

pie(!is.na(table(survival.df3$Household_Income)))
